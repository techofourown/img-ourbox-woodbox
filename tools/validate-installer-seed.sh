...
monitor_idx = next((i for i, item in enumerate(bootcmd) if "ourbox-installer-monitor.py" in str(item)), None)
ssh_bootstrap_idx = next((i for i, item in enumerate(bootcmd) if "ourbox-installer-ssh-bootstrap.sh" in str(item)), None)

if monitor_idx is None:
    print(
        f"ERROR: rendered seed {rendered_path} does not reference ourbox-installer-monitor.py",
        file=sys.stderr,
    )
    sys.exit(1)

if ssh_bootstrap_idx is None:
    print(
...
if monitor_idx > ssh_bootstrap_idx:
    print(
        f"ERROR: rendered seed {rendered_path} starts SSH bootstrap before installer monitor",
        file=sys.stderr,
    )
    sys.exit(1)
