...
#      or candidate-completion promotion handoff).
...
#      to a trusted event set: [published], [prereleased], or
...
while IFS= read -r wf; do
  ...
  # Is this an official publish/promote workflow? Detect by use of *-official.sh scripts.
  if ! grep -qE '(publish|promote)-(os|installer)-artifact-official\.sh' "${wf}"; then
    continue
  fi

  if grep -qE '^  workflow_dispatch:' "${wf}"; then
    fail "${name}: official publish/promote workflow exposes workflow_dispatch — official publication must only trigger from push-to-main, schedule, or candidate-completion promotion handoff"
  else
    PASS=$((PASS + 1))
  fi
done < <(find "${WORKFLOW_DIR}" -maxdepth 1 -name '*.yml' -o -name '*.yaml')
