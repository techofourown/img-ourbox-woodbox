...
tar -xOzf "${TARBALL}" ./airgap/platform/images.lock.json | grep -F '{}' >/dev/null
if tar -tzf "${TARBALL}" | grep -Fx './airgap/platform/catalog.json' >/dev/null; then
  echo "OS payload must not bake application catalog metadata into the substrate bundle" >&2
  exit 1
fi
if tar -tzf "${TARBALL}" | grep -Fx './airgap/platform/selected-apps.json' >/dev/null; then
