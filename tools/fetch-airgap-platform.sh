...
# Supported sources: OURBOX_AIRGAP_PLATFORM_REF env var > release/official-inputs.env
if [[ -n "${OURBOX_AIRGAP_PLATFORM_REF:-}" ]]; then
  REF="${OURBOX_AIRGAP_PLATFORM_REF}"
else
  INPUTS_ENV="${ROOT}/release/official-inputs.env"
  [[ -f "${INPUTS_ENV}" ]] || die \
    "Missing ${INPUTS_ENV} and OURBOX_AIRGAP_PLATFORM_REF is not set.
Legacy contracts/airgap-platform.ref fallback has been removed."
